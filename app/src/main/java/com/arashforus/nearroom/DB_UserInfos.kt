...
class DB_UserInfos(val context: Context?) : SQLiteOpenHelper(context, DB_NAME, null, VERSION) {

    private val TABLE_NAME = "UserInfos_TABLE"
    ...
    // Find new users in contacts and messages DB and insert in userinfo DB ////////////////////////
    fun init( myId : Int ){
        val db = DB_UserInfos(context).writableDatabase
        val cv = ContentValues()
        val newUserIds = ArrayList<Int>()
        val existUserIds = getAllIds()
        val contactsUserIds = DB_Contacts(context).getAllUserIds()
        val chatsUserIds = DB_Messages(context).findUniqueUserIds(myId)
        newUserIds.addAll(contactsUserIds)
        newUserIds.addAll(chatsUserIds)
        newUserIds.removeAll(existUserIds)
        val filteredNewUserIds = newUserIds.distinct()
        //println("new username add ................ "+filterednewusers)
    ...
    fun getPhoneInfo(){
        val db = DB_UserInfos(context).writableDatabase
        val cv = ContentValues()
        val users = DB_Contacts(context).getPhotoAndNames()
        users.forEach {
            cv.clear()
            cv.put(COLUMN_CONTACTNAME,it.fullName)
            cv.put(COLUMN_CONTACTPIC,it.photoUri)
            db.updateWithOnConflict(TABLE_NAME,cv," $COLUMN_ID = '${it.id}' ",null,SQLiteDatabase.CONFLICT_REPLACE)
        }
        if (db.isOpen) db.close()
    }
    ...
    fun getUsername( id : Int) : String {
        var username = ""
        val db = DB_UserInfos(context).readableDatabase
    ...
    private fun getAllUsernames() : ArrayList<String> {
    ...
    fun getProfilePhotosFromDB() : JSONObject{
        ...
        if (cursor.moveToFirst()) {
            do {
                json.put(cursor.getString(cursor.getColumnIndex(COLUMN_USERNAME)), cursor.getString(cursor.getColumnIndex( COLUMN_PROFILEPIC )))
            } while (cursor.moveToNext())
        }
        cursor.close()
        if ( db.isOpen) db.close()
    ...
    fun addUsername(username : String){
        val existUsernames = getAllUsernames()
    ...
    fun saveLastSeen(jsonObject : JSONObject) {
        val db = this.writableDatabase
        val cv = ContentValues()
        jsonObject.keys().forEach {
            cv.clear()
            cv.put(COLUMN_LASTSEEN,jsonObject.getString(it.toString()))
            db.updateWithOnConflict(TABLE_NAME,cv,COLUMN_USERNAME+" = '"+it as String+"'",null, SQLiteDatabase.CONFLICT_REPLACE)
        }
        if (db.isOpen) db.close()
    }
    ...
    fun getUserInfo(username : String) : Object_User {
    ...
    fun getUserInfo(id : Int) : Object_User {
        var info  = Object_User(null,null,"",null,null,null,null,null,null,null,0,0)
        val db = this.readableDatabase
        val cursor: Cursor = db.rawQuery("SELECT * from '$TABLE_NAME' WHERE $COLUMN_ID = '$id'", null)
        if ( cursor.count > 0 && cursor.moveToFirst() && !cursor.isNull(cursor.getColumnIndex(COLUMN_USERNAME)) ){
            info = Object_User(cursor.getInt(cursor.getColumnIndex(COLUMN_ID)),
                cursor.getInt(cursor.getColumnIndex(COLUMN_SERVERID)),
                cursor.getString(cursor.getColumnIndex(COLUMN_USERNAME)),
                cursor.getString(cursor.getColumnIndex(COLUMN_PHONENUMBER)),
            ...
            Volley_GetUserInfos(context!!, arrayListOf(id),object : Volley_GetUserInfos.ServerCallBack{
    ...
    fun setFavourite( userId : Int , favouriteState : Int) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.clear()
        cv.put(COLUMN_FAVOURITE,favouriteState)
        db.updateWithOnConflict(TABLE_NAME,cv," $COLUMN_ID = '$userId' ",null, SQLiteDatabase.CONFLICT_REPLACE)
        if (db.isOpen) db.close()
    }

    fun setMute( userId : Int , muteState : Int) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.clear()
    ...
    fun getMuteAndBlocked( userId : Int ) : JSONArray {
        val ja = JSONArray()
        val db = this.readableDatabase
        var muteStatus : Int? = null
        var blockStatus : Int? = null
        val cursor: Cursor = db.rawQuery("SELECT * from '$TABLE_NAME' WHERE $COLUMN_ID = '$userId'", null)
        if (cursor.moveToFirst()) {
            muteStatus = cursor.getIntOrNull(cursor.getColumnIndex(COLUMN_MUTE))
            blockStatus = cursor.getIntOrNull(cursor.getColumnIndex(COLUMN_BLOCKED))
        }
        if ( muteStatus == null ){
            ja.put(0)
        }else{
    ...
    companion object{
        const val DB_NAME: String = "UserInfos_DB"
